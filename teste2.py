...
def _request_img_from_api( comic_id:int) -> requests.models.Response:
    ...
    if comic_id % 100 == 0:
        print(comic_id)
    api_response = requests.get(url + str(comic_id) ,timeout=10, headers=HEADERS)
    if api_response.status_code != 200:
        print(f'[Warning] Error {api_response.status_code} in  request from comic id {comic_id}')
        return None

    comic_img_url = str(BeautifulSoup(api_response.content, 'html.parser').find('div', id='middleContainer').select('a')[-1].string)
    # print(comic_img_url)
    img_response = requests.get(comic_img_url ,timeout=10, headers=HEADERS)

    if img_response.status_code != 200:
        print(f'[Warning] Error {img_response.status_code} in IMGrequest from comic id {comic_id}')
        return None
